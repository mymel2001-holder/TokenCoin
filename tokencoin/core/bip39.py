"""
TokenCoin BIP39 Mnemonic Support
=================================
Implements BIP39 mnemonic code for generating deterministic wallets.
Uses the official BIP39 English wordlist (2048 words).

Key components:
  - Mnemonic: Generate, encode, decode mnemonic phrases
  - Seed: Derive binary seed from mnemonic + passphrase
  - Wordlist: The 2048 BIP39 English words
"""
...
# BIP39 English Wordlist (first 24 words shown, full list in production)
BIP39_WORDS: List[str] = [
    ...
    "plastic", "plate", "play", "player", "pleasure", "plenty", "plot", "pluck",
    ...
    "profit", "program", "project", "property", "proposal", "protect", "provide", "public",
    ...
    "stamp", "stand", "start", "state", "stay", "steak", "steel", "step",
    ...
    "sudden", "suffer", "sugar", "suggest", "suit", "sun", "sunny", "sunset",
    "super", "supply", "support", "suppose", "sure", "surface", "surgery", "surprise",
...
# Build lookup from word -> index
WORD_TO_INDEX: dict = {word: i for i, word in enumerate(BIP39_WORDS)}
...
class BIP39Mnemonic:
    ...
    def entropy_to_mnemonic(entropy: bytes) -> List[str]:
        ...
        if ent_bits not in (128, 160, 192, 224, 256):
            raise ValueError("Entropy must be 128-256 bits in multiples of 32")

        # Calculate checksum length
        cs_bits = ent_bits // 32

        # Compute checksum (SHA256 first byte)
        h = hashlib.sha256(entropy).digest()
        checksum = h[0] >> (8 - cs_bits)

        # Combine entropy + checksum
        entropy_int = int.from_bytes(entropy, "big")
        combined = (entropy_int << cs_bits) | checksum
    ...
    def mnemonic_to_entropy(words: List[str]) -> bytes:
        ...
        for word in words:
            ...
            combined = (combined << 11) | WORD_TO_INDEX[word]

        total_bits = len(words) * 11
        cs_bits = total_bits // 33  # 1 bit per 32 bits of entropy
        ...
        actual_cs = combined & ((1 << cs_bits) - 1)
    ...
    def mnemonic_to_seed(mnemonic: List[str], passphrase: str = "") -> bytes:
        """
        Convert mnemonic to 64-byte seed using PBKDF2 with HMAC-SHA512.
        seed = PBKDF2(mnemonic, passphrase, 2048, 64, HMAC-SHA512)
        """
        mnemonic_str = " ".join(mnemonic)
        salt = "mnemonic" + passphrase

        # PBKDF2 with HMAC-SHA512
        # 2048 iterations as per BIP39
    ...
    @staticmethod
    def generate_mnemonic(bits: int = 128) -> List[str]:
        """Generate a new random mnemonic phrase."""
        entropy = BIP39Mnemonic.generate_entropy(bits)
        return BIP39Mnemonic.entropy_to_mnemonic(entropy)

    @staticmethod
    def validate_mnemonic(words: List[str]) -> bool:
        """Validate a mnemonic phrase (checksum + wordlist)."""
        try:
            BIP39Mnemonic.mnemonic_to_entropy(words)
            return True
        except (ValueError, IndexError):
            return False

    @staticmethod
    def mnemonic_to_tkc_seed(mnemonic: List[str], passphrase: str = "") -> bytes:
        """
        Derive a TokenCoin-compatible 32-byte seed from a BIP39 mnemonic.
        Uses the BIP39 seed and then derives a TKC seed from it.
        """
        bip39_seed = BIP39Mnemonic.mnemonic_to_seed(mnemonic, passphrase)
        # Derive 32-byte TKC seed from the 64-byte BIP39 seed
        tkc_seed = hashlib.sha3_256(b"tokencoin_seed:" + bip39_seed).digest()
        return tkc_seed
...
def generate_mnemonic(bits: int = 128) -> str:
    """Generate a mnemonic phrase as a space-separated string."""
    words = BIP39Mnemonic.generate_mnemonic(bits)
    return " ".join(words)


def mnemonic_to_seed(mnemonic: str, passphrase: str = "") -> bytes:
    """Convert a mnemonic string to a TKC seed."""
    words = mnemonic.strip().lower().split()
    return BIP39Mnemonic.mnemonic_to_tkc_seed(words, passphrase)


def validate_mnemonic(mnemonic: str) -> bool:
    """Validate a mnemonic phrase string."""
    words = mnemonic.strip().lower().split()
    return BIP39Mnemonic.validate_mnemonic(words)
